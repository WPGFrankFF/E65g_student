...
public struct Grid: GridProtocol, GridViewDataSource {
    ...
    private func nextState(of pos: GridPosition) -> CellState {
        let iAlive = self[pos.row, pos.col].isAlive
        let numLivingNeighbors = neighborStates(of: pos).filter({ $0.isAlive }).count
        switch numLivingNeighbors {
        case 2 where iAlive,
             3: return iAlive ? .alive : .born
        default: return iAlive ? .died  : .empty
        }
    }
...
extension Grid: Sequence {
    ...
    public struct GridIterator: IteratorProtocol {
        private class GridHistory: Equatable {
            ...
            static func == (ls: GridHistory, rhs: GridHistory) -> Bool {
                return ls.position.elementsEqual(rhs.position, by: ==)
            }
            
            init(_ position: [GridPosition], _ previous: GridHistory? = nil) {
        ...
        private var Story: GridHistory!
        
        init(grid: Grid) {
            self.grid = grid
            self.Story = GridHistory(grid.living)
        }
        
        public mutating func next() -> GridProtocol? {
            if Story.hasCycle { return nil }
            let newGrid:Grid = grid.next() as! Grid
            Story = GridHistory(newGrid.living, Story)
            grid = newGrid
            return grid
        }
    }
...
class StandardEngine : EngineProtocol {
    ...
    static var eng: StandardEngine = StandardEngine(rows: 10,cols: 10) {
        didSet {
            if eng.rows != oldValue.rows {
                
                StandardEngine.eng.rows = eng.rows
                StandardEngine.eng.cols = eng.rows
    ...
    func changeEngineSize(size: Int) -> (){
        StandardEngine.eng.rows = size
        StandardEngine.eng.cols = size
        self.notifyDelageandPublishGrid()
        return StandardEngine.eng = StandardEngine(rows: size, cols: size)
    ...
    func changeIntialEngineGrid(GridData: [Array<Any>]) -> () {
        ...
        StandardEngine.eng = StandardEngine(rows: size1 + 1, cols: size1 + 1)
        ...
        for i in GridData {
            ...
            StandardEngine.eng.grid[x as! Int,y as! Int] = .alive
            self.notifyDelageandPublishGrid()
